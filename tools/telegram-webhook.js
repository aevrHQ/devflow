...
// Configuration
const DEFAULT_DOMAIN = "https://devflow.aevr.space";
...
async function getWebhookInfo(token) {
  ...
  try {
    ...
    if (result.ok) {
      ...
      console.log(
        `   Max Allowed Connections: ${info.max_connections || "default"}`,
      );
      console.log(
...
Examples:
  ...
  node telegram-webhook.js set YOUR_BOT_TOKEN https://devflow.aevr.space
...
async function main() {
  ...
  if (
    ...
    console.error(
      "   Provide as argument: node telegram-webhook.js <command> YOUR_BOT_TOKEN",
    );
    ...
    switch (command) {
      case "set":
        ...
        const allowedUpdates =
          allowedUpdatesIndex !== -1
            ? args[allowedUpdatesIndex + 1]
            : undefined;
